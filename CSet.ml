...
let rec offset o l =
  match l with [] -> [] | (c1, c2) :: r -> (c1 + o, c2 + o) :: offset o r
;;

let empty = []

let rec mem (c : int) s =
  match s with [] -> false | (c1, c2) :: rem -> if c <= c2 then c >= c1 else mem c rem
;;
...
let rec hash_rec = function [] -> 0 | (i, j) :: r -> i + (13 * j) + (257 * hash_rec r)
...
let one_char = function [ (i, j) ] when i = j -> Some i | _ -> None
